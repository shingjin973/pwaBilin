...
@Component({
    templateUrl: './wechat-course.component.html',
    styleUrls: ['./wechat-course.component.scss']
})
export class WechatCourseComponent implements OnInit {
    form: FormGroup;
    editForm: FormGroup;
    course: Course;
    lessons: Lesson[];
        ...
        try {
            swal.fire({
                ...
                if (result.value) {
                    ...
                    value.credits_per_session = parseFloat(value.credits_per_session);
                    value.session_duration = parseFloat(value.session_duration);                  
                    value.total_sessions = parseFloat(value.total_sessions);   
                    value.max_students = parseFloat(value.max_students); 
    ...
    createEditCourseForm() {
        this.editForm = this.fb.group({
            ...
            description_ch: new FormControl(this.course.description_ch, [Validators.required]),
    ...
    async saveCourse() {
        try {    
            ...
            value.teacher_rate_per_session = parseFloat(value.teacher_rate_per_session);
